...
var fourohfour = nock('http://404.mapbox.com').get('/16/46886/0.mvt').reply(404)
...
test('fails without zxy', function (t) {
    vt2geojson(
        ...
        function (err, result) {
            ...
            t.equal(
                err.message,
                'Could not determine tile z, x, and y from "./fixtures/9-150-194.mvt"; specify manually with -z <z> -x <x> -y <y>',
...
test('fails 404 response', function (t) {
    vt2geojson(
        ...
        function (err, result) {
            t.ok(err)
            t.ok(
                /Error retrieving data from/.test(err.message),
                'expected error message'
            )
            t.end()
        }
    ...
    vt2geojson(
        ...
        function (err, result) {
            ...
            result.features.forEach(function (f) {
                var val =
                    f.properties.vt_layer === 'landuse' ||
                    f.properties.vt_layer === 'poi_label'
                        ? true
                        : false
